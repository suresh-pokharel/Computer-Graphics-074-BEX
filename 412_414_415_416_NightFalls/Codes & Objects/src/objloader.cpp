...
coordinate::coordinate(float a, float b, float c) //This is the basic coordnates of vertices
...
face::face(int facen, int f1, int f2, int f3, int t1, int t2, int t3, int m) {  ////This structure stores every property of a face
    facenum = facen; //the number of the face 
    faces[0] = f1; //indexes for every vertex, which makes the face
    faces[1] = f2; //indexes for every vertex, which makes the face
    faces[2] = f3; //indexes for every vertex,which makes the face
    texcoord[0] = t1;  //texture coordinates for the face
    texcoord[1] = t2;   //texture coordinates for the face
    texcoord[2] = t3;   //texture coordinates for the face
    mat = m;
    four = false;  //checking if three or four faces
}

face::face(int facen, int f1, int f2, int f3, int f4, int t1, int t2, int t3, int t4, int m) {
    facenum = facen;
    faces[0] = f1; //same as above but constructor for four face cases
    faces[1] = f2;
    faces[2] = f3;
    faces[3] = f4;
    texcoord[0] = t1;
...
//structure for material for storing attributes of material
material::material(const char* na, float al, float n, float ni2, float* d, float* a, float* s, int i, int t)
{
    name = na; //name of the material is just number taken from mtl file
    alpha = al; 
    ni = ni2;
    ns = n;
    dif[0] = d[0]; //diffuse reflectance value
    dif[1] = d[1]; //diffuse reflectance value
    dif[2] = d[2]; //diffuse reflectance value

    amb[0] = a[0]; //ambient reflectance value
    amb[1] = a[1]; //ambient reflectance value
    amb[2] = a[2]; //ambient reflectance value

    spec[0] = s[0]; //specula reflectance value
    spec[1] = s[1]; //specula reflectance value
    spec[2] = s[2]; //specula reflectance value
                                                                                                                                                                                                                                                                                                                                                                                                                       ...
                                                                                                                                                                                                                                                                                                                                                                                                                       //all of the materials
        {
            ...
            for (int i = 0; i < tmp.size(); i++) //go through all lines of the mtllib file
            {
                ...
                else if (tmp[i][0] == 'm' && tmp[i][1] == 'a')      //and the texture
                {
                    ...
                    texture = loadTexture(filename,true);  //read the filename, and use the loadTexture function to load it, and get the id.
                    ismat = true;
                }
            }
...
//load the filename textures 
unsigned int objloader::loadTexture(const char* filename, bool generate=true)
{
    //nothing new in here
    unsigned int num;
    int w, h, comp;
...
/* Trying to use phong shading by hardcoding, not implemented properly
